...
template<typename T> static nlohmann::json DefaultSerialize(const T& elem, std::function<void(JSerError)>& pushError);
...
struct ForwardListSerializer
{
	...
	{
		return is_specialization<RawType, std::forward_list>();
	}
